...
class WordsController < ApplicationController
  ...
  def show
    key = Word.last['word_name']
   
    url = URI("https://api.dictionaryapi.dev/api/v2/entries/en/#{key}")
    ...
    @sv1 = result[0]['meanings']
    @sv2 = @sv1[0]['definitions']
    @sv3 = @sv2[0]['definition']
    @mp3=result[0]['phonetics']
    @x=@mp3[1]['audio']
  end
  ...
  def word_params
    params.require(:word).permit(:word_name, :word_def)
  end
end
